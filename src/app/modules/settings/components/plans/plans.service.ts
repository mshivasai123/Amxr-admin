...
export class PlansService {
  ...
  editPlan(data: any): Observable<any> {
    const endPoint = `api/subscription-plans/update/${data.id}`
    const req = { name: data.name, days: data.days,currencyType : data.currencyType,actualCost : data.actualCost,discountInPercentage:data.discountInPercentage }
